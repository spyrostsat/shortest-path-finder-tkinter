...
class ShortestPathFinder(object):
    ...
    def initial_configurations(self):
        ...
        screen_multiplier = 0.9  # we will use this variable to adjust the size of the root window
    ...
    def __init__(self) -> None:
        ...
        self.attraction_repulsion_combobox.current(1)
    ...
    def start_game(self):
        ...
        self.graph = nx.Graph()  # we will use a not directed graph since all edges are bidirectional
        ...
        for i in range(self.rows):
            ...
            for j in range(self.columns):
                ...
                # We will use the coordinates of the button as the name of the node. We will also add the weight of the respective area as an attribute to
                # the node to know the cost of each node. Each node will be represented as a tuple (row, column)
        ...
        self.instructions_label = tk.Label(self.bottom_frame, text="First, click where to set the starting point", font=(self.font, self.font_size), bg=self.bg, fg=self.fg)
    ...
    def change_area(self, event):
        ...
        if self.clicked_buttons_count == 1:  # the first button that the user clicks will be the start point
            ...
            self.instructions_label.configure(text="Now, click where to set the end point")
            ...
            self.instructions_label.configure(text=f"Finally, click where to add some obstacles. Add at least {self.min_obstacles} obstacles.")
        
        else:
            # Let's change the color of the button to represent an obstacle
            button.configure(bg=self.obstacle_color, text="X")
            self.obstacles.append((button_row, button_column))
    ...
    def find_shortest_path(self):
        ...
        # Let's destroy everything that is in the root window
        ...
        for i in range(self.rows):
            for j in range(self.columns):
                # Let's check if the button is an obstacle
                if (i, j) in self.obstacles:
                    # Let's remove the node from the graph
        ...
        for i in range(self.rows):
            for j in range(self.columns):
                ...
                if (i, j) not in self.obstacles:
                    ...
                    # Let's check if the button is not in the first row
                    if i != 0:
                        # Let's check if the button above is not an obstacle
                        if (i - 1, j) not in self.obstacles:
                            # Let's add an edge between the two nodes
                            self.graph.add_edge((i, j), (i - 1, j), weight=self.graph.nodes[(i - 1, j)]['weight'])
                    
                    # Let's check if the button is not in the last row
                    if i != self.rows - 1:
                        # Let's check if the button below is not an obstacle
                        if (i + 1, j) not in self.obstacles:
                            # Let's add an edge between the two nodes
                            self.graph.add_edge((i, j), (i + 1, j), weight=self.graph.nodes[(i + 1, j)]['weight'])
                    
                    # Let's check if the button is not in the first column
                    if j != 0:
                        # Let's check if the button on the left is not an obstacle
                        if (i, j - 1) not in self.obstacles:
                            # Let's add an edge between the two nodes
                            self.graph.add_edge((i, j), (i, j - 1), weight=self.graph.nodes[(i, j - 1)]['weight'])
                    
                    # Let's check if the button is not in the last column
                    if j != self.columns - 1:
                        # Let's check if the button on the right is not an obstacle
                        if (i, j + 1) not in self.obstacles:
                            # Let's add an edge between the two nodes
                            self.graph.add_edge((i, j), (i, j + 1), weight=self.graph.nodes[(i, j + 1)]['weight'])
        ...
        try:
            self.shortest_path = nx.shortest_path(self.graph, self.start_point, self.end_point, weight="weight")
        except nx.exception.NetworkXNoPath:
            if messagebox.askyesno("Warning", "There is no path between the start point and the end point! Play again?") == True:
                self.play_again()
            else:
                self.root.destroy()
        ...
        shortest_path_dijkstra_string = "Dijkstra shortest path: "
        for node in self.shortest_path:
            shortest_path_dijkstra_string += f"{node} -> "
        # remove the last arrow
        shortest_path_dijkstra_string = shortest_path_dijkstra_string[:-3]
        
        # Let's add some lines changes to the string to make it look better if the string is too long and make sure the string separates in a closing parenthesis
        limit = 200
        ...
        cost = 0
        for i in range(1, len(self.shortest_path) - 1):
            cost += self.graph.nodes[self.shortest_path[i]]['weight']
        
        
        # Let's color the path
        for i in range(len(self.shortest_path)):
            self.map[self.shortest_path[i][0]][self.shortest_path[i][1]].configure(bg=self.path_color)
        
        # Let's create a frame to hold the buttons
        self.bottom_frame = tk.Frame(self.root, bg=self.bg)
        self.bottom_frame.pack(pady=self.base_padding)
        
        # Let's create a label to tell the user the cost of the path
        self.cost_label = tk.Label(self.bottom_frame, text=f"{shortest_path_dijkstra_string}. Cost: {cost}", font=(self.font, int(0.8 * self.font_size)), bg=self.bg, fg=self.fg)
        self.cost_label.pack(pady=self.base_padding)
        ...
        self.play_again_button.pack(pady=self.base_padding)
        ...
        self.exit_button.pack(pady=self.base_padding)
